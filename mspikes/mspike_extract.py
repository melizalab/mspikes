...
def klusters_extraction(arffile, log=extractor._dummy_writer, **options):
    ...
    with arf.file(arffile,'r') as arfp:
        with klusters.klustersite(basename, **options) as ks:
            # TODO handle entries without sample_count attributes
            ...
            for channel,thresh,maxrms in zip(channels,threshs,rmsthreshs):
                ...
                for entry, times, spikes, Fs in gen:
                    if times is None:
                        ...
                        # verify that Fs is the same for all entries
                        if hasattr(ks,'samplerate'):
                            if ks.samplerate != Fs: raise RuntimeError, "entries have different sampling rates"
                        else:
                            ks.samplerate = Fs
